...
// Blog Post Model
class BlogPost {
    constructor(filename, frontMatter, content) {
        this.filename = filename;
        this.slug = filename.replace(/\.md$/, '');
        this.title = frontMatter.title || 'Untitled';
        this.date = frontMatter.date || '';
        this.author = frontMatter.author || 'Admin';
        this.tags = (frontMatter.tags || '').split(',').map(t => t.trim()).filter(t => t);
        this.excerpt = frontMatter.excerpt || this.generateExcerpt(content);
...
class BlogManager {
    ...
    async loadPosts() {
        // In a real implementation, this would fetch from a manifest or directory listing
        // For now, we'll load from a JSON file generated by the build process
        try {
    ...
    async loadPost(filename) {
        try {
            ...
            const post = new BlogPost(filename, frontMatter, body);
...
function renderBlogListing(posts, container) {
    ...
    const html = posts.map(post => `
        <article class="bg-mesh-card rounded-xl p-6 border border-slate-800 hover:border-mesh-500/50 transition-colors mb-6 group">
            ...
            <a href="post.html?slug=${post.slug}" class="text-sm font-semibold text-mesh-500 hover:text-mesh-300">Read More &rarr;</a>
        </article>
    `).join('');
